...
if(touchedNode != self){
 ...
 /* Level eleven no boundaries code */
...
//        
//        dispatch_async(dispatch_get_main_queue(), ^(){
//            [self detectCollisionsInTimeInterval:0.4 ofNodeArray:self.children];
//        });
//        
//        
//        
...
//}
//
////since this is a no boundaries level, detect if the ball is in the edge of the scene
////if it is, since when it goes off bounds it'll be removed from the scene no memory leaks
////when the ball goes of the edge create a new ball from the opposite side.
//-(void) detectBoundingBoxCollisions:(NSArray*)ballArray
//{
//    
//    for (BallNode *ball in ballArray){
//        
//        if ([ball isKindOfClass:[BallNode class]]) {
//            CGPoint ballPosition = ball.position;
//            
//            if (ballPosition.x + ball.frame.size.width/2 >= self.scene.size.width || ballPosition.y + ball.frame.size.width/2 >= self.scene.size.height) {
//                
//                BallNode *newBall = [[BallNode alloc] init];
//                newBall.position = CGPointMake(remainderf(ballPosition.x, self.scene.size.width) , remainderf(ballPosition.y, self.scene.size.height));
//                newBall.ballColor = ball.ballColor;
//                
//                [self addChild:newBall];
//                [newBall.physicsBody applyImpulse:ball.physicsBody.velocity];
//                
//                
//            }
//            
...
//}
//
//-(void)detectCollisionsInTimeInterval:(NSTimeInterval)timeInterval ofNodeArray:(NSArray*)nodeArray{
//    
//    
//    [self performSelector:@selector(detectBoundingBoxCollisions:) withObject:nodeArray afterDelay:timeInterval];
//    
//    
//}
//
